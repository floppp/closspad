...
(def importance-bg-color
  {:major "bg-green-300"
   :p1 "bg-teal-300"
   :p2 "bg-sky-300"
   :promises "bg-indigo-300"
   :regular "bg-white"
   :undefined "bg-white"})

(defn get-importance-color
  [importance]
  (-> importance m/importance->type importance-bg-color))
...
(defn- component
  ...
  (let [f (first result)
    ...
    [:div.relative.border.border-gray-200.rounded-lg.p-4.shadow-sm
     ...
     [:div.absolute.bg-white.px-2.rounded-sm
      {:style {:position "absolute"
               :top "-0.75rem"
               :right "1rem"}
       :class [(get-importance-color importance)]}
      (m/importance->name importance)]]))
