// Unbounded Elo Rating System (no upper limit, starts at 0)
// Based on original Elo system but adapted for unlimited points growth

const { UNBOUNDED_ELO_DEFAULTS } = require('../shared/constants');
const { determineWinner } = require('../shared/utils');

const defaultOptions = UNBOUNDED_ELO_DEFAULTS;
...
function proximityFactor(system, player) {
    ...
    // Sigmoid normalization for unbounded ratings
    // As rating increases, proximity approaches 0
    // At rating = 0, proximity = 2 (maximum effect)
    // At rating = system.proximityCalculationMax, proximity ≈ 0.5
    const normalized = 2 / (1 + Math.exp(rating / system.proximityCalculationMax));
    
    // Ensure minimum proximity of 0.1 to maintain some effect
...
function clampRating(system, rating) {
    // Only enforce minimum bound, no upper bound
    return Math.max(system.minRating, rating);
}
...
function computeVariationPerPlayer(system, couple, coupleRating, otherCoupleRating, isWinner, expectedWin, importance) {
    ...
    for (const playerId of couple) {
        ...
        // Cap underdog bonus to prevent excessive bonuses with large gaps
        const cappedGap = Math.min(underdogGap, 100);
        const underdogBonus = isWinner ? Math.round(cappedGap * 0.015) : 0;
...
// Factory function for Unbounded Elo system
function createEloUnboundedSystem(options = {}) {
