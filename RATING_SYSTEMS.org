...
- **Characteristics**: Simple additive (10 × importance)
...
### ✅ COMPLETED (Toggle Integration)
- [x] Update ClojureScript integration to use toggle
- [x] Modify `full-matches-process` to accept system type parameter
- [x] Update toggle to switch between bounded/unbounded Elo
- [x] Add `:rating-system :recalculate-all` event for toggle
- [x] Update network/domain.cljs to use bounded Elo by default
- [x] Test and verify backward compatibility
...
```
src/js/
├── ratingSystem.js              # Backward compatibility wrapper
├── ratingSystemFactory.js       # Factory interface
├── shared/
│   ├── constants.js            # System defaults
│   └── utils.js               # Shared utilities
└── ratingSystems/
    ├── eloSystem.js           # Bounded Elo (0-100)
    ├── eloUnboundedSystem.js  # Unbounded Elo (0-∞)
    └── atpSystem.js           # ATP-like (additive)
```
...
### Current State
- Toggle exists in classification view: "Oficial" vs "Llorón"
- Toggle state: `:ui/toggle-value` in app state
- Default: `nil` (falsy) → Should use bounded Elo

### Planned Integration
1. **Toggle mapping**:
   - Falsy (`false`/`nil`) → `'elo'` (bounded, 0-100)
   - Truthy (`true`) → `'elo-unbounded'` (unbounded, starts at 0)
...
3. **Labels** (to be decided):
   - Keep "Oficial"/"Llorón" or change to "Elo (0-100)"/"Elo (∞)"

## Testing Results

### ✅ Unbounded Elo System Works
- Points start at 0
...
### Risks Mitigated
- Mathematical instability with high ratings → Sigmoid functions
- Division by zero with 0 points → `Math.max(points, 0.1)`
- Performance with large numbers → Capped bonuses
- User confusion → Clear system separation